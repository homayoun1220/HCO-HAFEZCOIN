...
\begin{document}
	...
	\begin{abstract}
		...
		design to resource design. Within mechanisms preserving the
		relevant parallelizability properties, decentralization limits
		arise from resource structure rather than consensus rules alone. The same properties also enable concentrated
	...
	concentration with reusable stock cost $O(s)$---the
	$T$-scaling renewal cost $C_{\mathrm{flow}}$ vanishes
	once stock is acquired---whereas throughput-bounded
	...
	separation and the Resource Substitution Theorem.
	...
	stock is acquired, so the reusable resource component requires only
	$O(s)$ stock cost; any additional dependence on
	the time horizon arises through coordination overhead
	$h(s,T)$ rather than resource renewal.
	...
	is established under an additive cost decomposition
	$C(s,T) = C_p(s,T) + C_t(s,T)$, where a throughput-bounded
	...
	The open question is not whether the theorem extends
	to overheads that remain $o(sT)$---Theorem~\ref{thm:impossible}
	already covers that class---but rather how to
	realistically characterize coordination processes in
	practice: what functional forms of $h(s,T)$ arise in
	deployed systems, and how marginal-cost behavior
	$\Delta(s,T)$ varies under realistic coordination
	assumptions beyond the $O(s+T)$ benchmark.
	...
	More broadly, these results suggest that within
	mechanisms preserving the relevant parallelizability
	properties, decentralization limits arise primarily
	from resource structure rather than consensus rules
	alone.
